...
This document lists the current maintainers of the Anamnesis project and
describes the maintainer role.
...
** *Responsibilities*: Overall direction, architecture, major decisions,
release management
...
_(None yet - accepting applications! See "`Becoming a Maintainer`"
below)_
...
* Alignment with Palimpsest Principles (reversibility, emotional safety,
...
* Label issues appropriately (`+bug+`, `+feature+`, `+help wanted+`,
etc.)
...
. *Release Management* (Lead Maintainer)
* Coordinate releases
* Update CHANGELOG.md
...
Maintainers have: - *Write access* to the repository (direct push to
feature branches, not `+main+`) - *Merge authority* for PRs in their
domain - *Veto power* on changes that violate project principles (with
explanation) - *Vote* on major decisions (1 vote per maintainer, simple
majority)

*Consensus vs. Voting*: - Prefer *consensus* (everyone agrees or is
willing to try) - Use *voting* only when consensus can’t be reached -
Lead Maintainer has *tiebreaker vote*

*Escalation*: - If maintainers disagree, discuss in private maintainer
channel - If still stuck, escalate to Lead Maintainer for final call -
...
. *Collaborator* (10+ merged PRs, 6+ months activity)
...
* *Communication*: Responsive (48-hour response time for critical
issues)

==== Nomination Process

[arabic]
. *Nomination*: Maintainer opens private discussion with other
maintainers
. *Notification*: Nominee informed and invited to accept
. *Public Announcement*: If accepted, nominee announced in GitHub
Discussion
. *Community Feedback*: 2-week period for questions/concerns
. *Vote*: Maintainers vote (simple majority, must be public with
rationale)
. *Onboarding*: New maintainer granted access, added to MAINTAINERS.md
. *Announcement*: Public welcome announcement
...
* *Inactive*: No activity for 6+ months (automatic removal with
notification)
* *Forced*: Code of Conduct violation, abuse of authority (vote
required)

==== Process

[arabic]
. *Notification*: Email other maintainers (for voluntary)
. *Transition*: Hand off active responsibilities
. *Access Revocation*: Remove write access, maintainer team membership
. *Recognition*: Move to "`Emeritus Maintainers`" section below
. *Announcement*: Public thank-you announcement

Emeritus maintainers may return if circumstances change (simplified
re-nomination).

=== Emeritus Maintainers

_(None yet - this section will recognize past maintainers who stepped
down gracefully)_
...
Meetings will be: - *Scheduled in advance* (with calendar invite) -
...
Maintainer discussions are *public by default* unless: - Security
vulnerabilities (pre-disclosure) - Code of Conduct enforcement -
Personal matters (health, family, etc.) - Legal issues
...
[arabic]
. *Direct communication*: Talk it out, assume good faith
. *Mediation*: Another maintainer mediates discussion
. *Lead Maintainer decision*: Final call if mediation fails
. *Community input*: For major disagreements, seek community feedback
...
*Last updated*: 2025-11-22 *Next review*: Q2 2026 (or when 3+
maintainers active)

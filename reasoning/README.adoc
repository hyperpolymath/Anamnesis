...
Meta-reasoning about conversation structure, artifact lifecycles, and
cross-conversation relationships using higher-order logic programming.

=== Purpose

Use λProlog’s Higher-Order Abstract Syntax (HOAS) and meta-reasoning
capabilities to:
...
λProlog uses the meta-language’s binding mechanisms for object-language
bindings. This makes representing conversation context natural:
...
* https://lpcic.github.io/elpi/[ELPI Documentation]
* https://www.lix.polytechnique.fr/Labo/Dale.Miller/lProlog/[λProlog
Tutorial]
...
. Create artifact lifecycle module
. Add contamination detection
. Implement episodic memory queries
. Test on proving-ground conversations
. Integrate with OCaml/Elixir via port

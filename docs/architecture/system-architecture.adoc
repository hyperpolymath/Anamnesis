...
* **AffineScript visualization** - Type-safe, functional UI components
...
| **Type safety**
| Strong typing in OCaml, AffineScript, Elixir typespecs, Julia
...
==== AffineScript for Visualization
...
* **affinescript-recharts** - Timeline/lifecycle charts
* **Kill JavaScript** - User goal: AffineScript → native (future)
...
└────────────────────────────┬────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│           Visualization Layer (AffineScript + React)                │
│  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────┐ │
│  │ Conversation     │  │ Artifact         │  │ Project      │ │
│  │ Graph (Reagraph) │  │ Timeline (Gantt) │  │ Category Viz │ │
│  │ WebGL, 2D/3D     │  │ affinescript-recharts│  │ Fuzzy bounds │ │
│  └──────────────────┘  └──────────────────┘  └──────────────┘ │
│  ┌──────────────────────────────────────────────────────────┐  │
...
[source,text]
----
1. User Query (AffineScript UI)
   ↓
2. [AffineScript → Elixir] HTTP POST with query parameters
   ↓
3. [Elixir] Parse query, determine data needs
   ...
   └─ Complex reasoning?
      ...
      ↓
      [Elixir → AffineScript] JSON response
      ↓
      [AffineScript] Render visualization
----
...
11. [Julia → Virtuoso] Store predictions in graph:predictions:uuid
    ↓
12. [AffineScript UI] Visualize predictions alongside actual data
----
...
GRAPH <anamnesis:categories:12345> {
    ...
    anamnesis:conv:12345 anamnesis:belongsTo anamnesis:proj:affinescript-evangeliser ;
...
=== Visualization Layer (AffineScript)
...
│   │   ├── Recharts.res           (* affinescript-recharts wrapper *)
...
module ColorMixing = {
  ...
  let projectColors = Map.String.fromArray([
    ...
    ("affinescript-evangeliser", {r: 230, g: 74, b: 25}),  // Orange/Red
...
Load Balancer (nginx)
    ...
    │  └─ Julia Port
    │
    ├─ Static Assets (AffineScript compiled JS)
    │
    └─ Virtuoso Cluster
       ├─ Primary (read/write)
       └─ Replica (read-only)
...
* **Mobile client** - AffineScript Native for iOS/Android
...
* **AffineScript** for type-safe, functional visualization

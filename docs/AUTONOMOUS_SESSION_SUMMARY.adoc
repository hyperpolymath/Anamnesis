...
4. **AffineScript Visualization** (`docs/research/affinescript-visualization.adoc`)
   * Reagraph (WebGL): 1000-5000 nodes, 2D/3D force-directed
   * affinescript-recharts: Mature bindings for timelines
...
3. **Component READMEs**
   ...
   * Visualization: AffineScript, type safety, WebGL rendering
...
**Files Created**:
- `visualization/bsconfig.json` - AffineScript compiler config
...
- **Component READMEs**: 6
- **Languages**: OCaml, Elixir, Julia, λProlog, AffineScript
...
2. **Build infrastructure**
   ...
   * AffineScript: `npm install && npm run res:build`
...
- AffineScript visualization components (Reagraph bindings)
- Phoenix HTTP API
...
- Type safety enforced at compile time (OCaml, AffineScript)

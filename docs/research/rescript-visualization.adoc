= AffineScript Visualization Ecosystem Research
...
[abstract]
--
Comprehensive research into the AffineScript visualization ecosystem for the Anamnesis project. This document evaluates libraries, patterns, and approaches for visualizing conversation graphs, artifact lifecycles, multi-project categorization, and episodic memory structures using AffineScript's type-safe functional paradigm.
--
...
The AffineScript visualization ecosystem is *nascent but viable*. While there are limited native AffineScript visualization libraries, the language's excellent JavaScript interoperability and strong type system make it well-suited for creating type-safe bindings to established React visualization libraries.

*Recommended Approach:* Use **affinescript-recharts** for standard charts and timelines, with custom bindings to **Reagraph** (WebGL graph visualization) for conversation networks. Leverage AffineScript's type system to ensure visualization code correctness at compile time.
...
|Charts & Timelines
|**affinescript-recharts**
|Mature AffineScript bindings, declarative API, good for artifact lifecycles

|UI Framework
|**affinescript-react** + **affinescript-core**
...
* ✅ Type safety throughout (AffineScript's strength)
...
== AffineScript Ecosystem Overview

=== Language Characteristics

AffineScript is a robustly typed functional programming language that compiles to JavaScript with the following characteristics relevant to visualization:
...
New standard library released with AffineScript 11, shipped as `@affinescript/core`. Fusion of `affinescript-js`, `affinescript-promise`, and Belt. Will be included with `affinescript` package in future versions.

**affinescript-react** (React Bindings)::
Official React bindings with excellent type safety and idiomatic AffineScript patterns. Compiles closely to idiomatic JS code, making integration seamless.
...
**Status:** JavaScript library, requires custom AffineScript bindings +
...
* **TypeScript** definitions (can be translated to AffineScript types)

==== Example AffineScript Binding Pattern
...
**Status:** `affinescript-d3` package exists (alpha, 4 years old) +
**Repository:** https://www.npmjs.com/package/affinescript-d3 +
...
> "Rendering code would remain in JS due to heavy usage of the D3 library. Rewriting it in AffineScript would mean either adding typed bindings for every D3 function used, or using an untyped binding, which misses the whole point of migrating to AffineScript (type-safety)."
...
=== Recommended: affinescript-recharts

**Status:** Mature AffineScript bindings +
**Repository:** https://github.com/Minnozz/affinescript-recharts +
**NPM Packages:** `@minnozz/affinescript-recharts`, `@float-capital/affinescript-recharts` +
**License:** Fork of `ahrefs/bs-recharts`, updated to new AffineScript syntax

==== Key Features

* **Declarative API** - Idiomatic AffineScript/React patterns
...
module SvarGantt = {
  type task = {
    ...
    text: string,
    start: Js.Date.t,
    end_: Js.Date.t, // 'end' is reserved in AffineScript
...
=== AffineScript-React Foundation

**Status:** Official bindings, actively maintained +
**Documentation:** https://affinescript-lang.org/docs/react/latest/introduction +
**Maturity:** Production-ready

==== Philosophy

AffineScript-React compiles closely to idiomatic JavaScript, making integration with existing React ecosystems seamless. This is crucial for visualization work where you'll be mixing:

* AffineScript components (type-safe logic)
* JavaScript visualization libraries (via bindings)
* Standard React patterns (hooks, context, etc.)

=== Native AffineScript UI Libraries
...
AffineScript UI ecosystem is *small*. Philosophy: use JavaScript libraries via bindings rather than rebuilding.

=== Recommended Approach: JavaScript UI Libraries via Bindings

==== Chakra UI (via Custom Bindings)

Mentioned in AffineScript forums as recommended option.

* Component-based
* Accessible by default
* Themeable
* Good TypeScript definitions (translatable to AffineScript)

==== Tailwind CSS (via affinescript-tailwind or similar)

* Utility-first CSS
* Works well with AffineScript's functional approach
* No heavy bindings needed
* Style composition via string interpolation or dedicated bindings

==== Material-UI (Existing Bindings)

**Status:** `affinescript-material-ui` package exists +
...
==== FRP in AffineScript Context

While pure FRP libraries (like RxJS) aren't common in AffineScript ecosystem, React's hooks provide a pragmatic reactive approach:
...
Without type safety, errors emerge at *runtime* when users interact. AffineScript catches these at *compile time*.
...
AffineScript's immutability helps but monitor:
...
Most WebGL libraries (Reagraph, Graph.gl) provide built-in zoom/pan. AffineScript bindings:
...
== Real-World AffineScript Visualization Examples

=== Community Projects

From research, AffineScript visualization projects are rare in public repositories. Most production AffineScript is proprietary.

==== 1. AffineScript React Native SVG

**Repository:** https://github.com/affinescript-react-native/svg
...
==== 2. affinescript-recharts Examples
...
==== 3. Hyperswitch (Production AffineScript)

**Blog Post:** https://hyperswitch.io/blog/why-we-chose-affinescript-over-typescript

Large-scale production AffineScript application. While not primarily visualization, demonstrates:

* Complex state management
* Integration with JavaScript ecosystem
* Performance at scale
* Why companies choose AffineScript over TypeScript
...
* **AffineScript Manual** - https://affinescript-lang.org/docs/manual/latest/introduction
* **AffineScript-React Docs** - https://affinescript-lang.org/docs/react/latest/introduction
* **Bindings Cookbook** - https://github.com/rescriptbr/affinescript-bindings-cookbook

==== Migration Stories

* **"Rewriting a Project in AffineScript"** - https://yangdanny97.github.io/blog/2021/07/09/Migrating-to-Rescript
* **"From TypeScript to AffineScript"** - https://www.greyblake.com/blog/from-typescript-to-affinescript/

==== Community

* **AffineScript Forum** - https://forum.affinescript-lang.org/
* **AffineScript Discord** - Active community for questions
* **awesome-affinescript** - https://github.com/fhammerschmidt/awesome-affinescript

=== Recommended Learning Path for Anamnesis

. **Master AffineScript basics** - Type system, pattern matching, modules
. **Learn affinescript-react** - Hooks, components, state management
. **Study affinescript-recharts source** - Understand binding patterns
...
[source]
----
visualization/                    # AffineScript visualization layer
├── src/
│   ├── bindings/                # JavaScript library bindings
│   │   ├── Reagraph.res         # Graph visualization bindings
│   │   ├── Recharts.res         # Chart bindings (use @minnozz/affinescript-recharts)
...
│
├── bsconfig.json                # AffineScript configuration
...
JSON API
    ↓
AffineScript Visualization Layer
    ↓ (load)
...
Keep AffineScript types in sync with Elixir schemas:
...
* Tool generates AffineScript types (doesn't exist yet, would need to build)

**Option C: Runtime Validation**

* Use AffineScript decoders (like `@glennsl/affinescript-json-combinators`)
...
AffineScript's type system IS your unit tests for many cases:
...
[source,affinescript]
----
// Use affinescript-testing-library or similar
...
. ✅ Setup AffineScript project structure
. ✅ Install dependencies (`affinescript`, `@affinescript/react`, `@minnozz/affinescript-recharts`)
...
. ✅ Study affinescript-recharts for binding patterns
...
The AffineScript visualization ecosystem is **viable but requires custom binding work**. The language's strengths—type safety, functional paradigm, excellent JavaScript interop—align perfectly with Anamnesis's requirements.

=== Key Takeaways

. **Use affinescript-recharts** for standard charts and timelines (mature, well-maintained)
. **Write custom Reagraph bindings** for graph visualization (WebGL performance crucial)
. **Leverage AffineScript's type system** to prevent runtime visualization errors
...
The path forward is clear: **AffineScript + Reagraph + affinescript-recharts** forms a solid foundation for Anamnesis's visualization needs.

== References

=== Official Documentation

* AffineScript Language Manual: https://affinescript-lang.org/docs/manual/latest/introduction
* AffineScript React: https://affinescript-lang.org/docs/react/latest/introduction
* AffineScript Bindings Cookbook: https://github.com/rescriptbr/affinescript-bindings-cookbook
...
* "Why AffineScript Was a Better Fit Than TypeScript for Hyperswitch": https://hyperswitch.io/blog/why-we-chose-affinescript-over-typescript
* "Rewriting a Project in AffineScript": https://yangdanny97.github.io/blog/2021/07/09/Migrating-to-Rescript
* "From TypeScript to AffineScript": https://www.greyblake.com/blog/from-typescript-to-affinescript/
